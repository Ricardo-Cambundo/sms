...
const SignUp = ({handleScroll}) => {
    ...
    const validateFull = () => {
        return new Promise((resolve) => {
            if (fullname === '') {
                setFullError('Nome da organização é necessário.');
                resolve(false);
            }  else {
                setFullError('');
                resolve(true);
    ...
    const validateEmail = () => {
        return new Promise((resolve) => {
            ...
            if (email === '') {
                setEmailError('Nome de remetente é necessário.');
                resolve(false);
            } else {
                setEmailError('');
                resolve(true);
    ...
    const validatePhone = () => {
        return new Promise((resolve) => {
            if (phone === '') {
                setPhoneError('NIF é necessário.');
                resolve(false);
            } else {
                setPhoneError('');
                resolve(true);
    ...
    const next = async () => {
        ...
        if (validations.every(v => v === true)) {
            ...
            setLoading(true)
            setTimeout(() => {
                AsyncStorage.setItem('organizacao', JSON.stringify({
                    nome: fname,
                    nif: phone,
                    remetente: email
                }))
        ...
        <KeyboardAwareScrollView keyboardShouldPersistTaps="handled" style={{flex: 1, height: '100%', backgroundColor: '#003a75', }}>
            ...
            <View style={[styles.header]}>
                ...
                <ScrollView keyboardShouldPersistTaps="handled" style={{flex: 1}} scrollEnabled={false}>
                    ...
                    <Text style={styles.title1}>Informações da Organização</Text>
                                ...
                                <Pressable onPress={()=> {
                                                                    ...
                                                                    <View style={styles.inputContainer}>
                                                                                <Icon style={styles.icon} name='person-outline' size={18}/><View style={{
                                                                                    ...
                                                                                    <Text style={styles.label}>Nome de remetente</Text>
                                                                                    <TextInput ref={emailRef} style={styles.input} value={email} onChangeText={(value)=>setEmail(value)}/>
                                                                                </View>
                                                                            </View>
                                                                </Pressable>
